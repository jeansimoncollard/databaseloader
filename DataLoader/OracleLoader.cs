...
namespace DatabaseLoader
{
    ...
    public class OracleLoader : IOracleLoader
    {
        ...
        public void Unload(string connectionString, string filePath)
        {
            ...
            var query = getDeleteQuery(fileContent);
            try
            {
        ...
        {
            var fileContent = File.ReadAllText(filePath);
            executeInsertQuery(connectionString, getInsertQuery(fileContent), executeDeleteOrInsertQueryDeferConstraints);
        }

        private void conventionalLoadPermanent(string connectionString, string filePath)
        {
            var fileContent = File.ReadAllText(filePath);
            executeInsertQuery(connectionString, getInsertQuery(fileContent), executeDeleteOrInsertQuery);
        }

        private string getInsertQuery(string fileContent)
        {
        ...
        private string getDeleteQuery(string fileContent)
        {
            var insertQueryLength = getLengthOfInsertQuery(fileContent);
